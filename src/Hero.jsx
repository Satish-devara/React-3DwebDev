...
function Hero(){
    ...
    return(
        <div className="relative h-dvh w-screen overflow-x-hidden">
                {isLoading && (
                    <div className="felx-center absolute z-[100] h-dvh w-screen overflow-hidden bg-violet-50 ">
            ...
            <div className="mask-clip-path absolute-center absolute z-50 size-64 cursor-pointer overflow-hidden rounded-lg">
                <div onClick={handleMiniVdClick} className="origin-center scale-50 opacity-0 transition-all duration-500 ease-in hover:scale-100 hover:opacity-100">
                   <video src={getVideoSrc(upComingVideoIndex)} ref={nextVideoRef}
                   loop
                   muted
                   autoPlay
                   playsInline
                   id="current-video"
                   className="size-64  top-1/2 left-1/2 origin-center scale-150  object-center object-cover"
            ...
            <video src={getVideoSrc(currentIndex === totalVideos? 1 : currentIndex )}
            loop autoPlay muted
            playsInline className="absolute left-0 top-0 size-full object-cover object-center"
            onLoadedData={handleVideoLoad}></video>
            </div>
